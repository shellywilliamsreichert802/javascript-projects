...
strings = [food, equipment, pets, sleepAids];

//1) Use split to convert the strings into four cabinet arrays. Alphabetize the contents of each cabinet.
console.log(strings[0].split(',').sort());
console.log(strings[1].split(',').sort());
console.log(strings[2].split(',').sort());
console.log(strings[3].split(',').sort());
...
let n = input.question("Select a cabinet (0-3) space sin the cargoHold: ");

//4) Use bracket notation and a template literal to display the contents of the selected cabinet. If the user entered an invalid number, print an error message.
if (n > cargoHold.length-1) {
